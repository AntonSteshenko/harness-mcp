...
- [X] T012 [P] Move the fixed `AGENTS_MD_TEMPLATE`/`INIT_SKILL_MD_TEMPLATE` content out of `frontend/lib/os/init.ts`'s inline string literals into standalone Markdown files (FR-008, FR-009, research.md §5), per user request: created `frontend/lib/os/templates/AGENTS.md` and `frontend/lib/os/templates/init.md` with the identical content; `init.ts` now reads both via `fs.readFileSync(path.join(process.cwd(), "lib/os/templates/<name>"), "utf-8")` at module load and exports the same two constant names, so `initializeCompanyOs` (T001) is otherwise unchanged. `npx tsc --noEmit` clean; content verified byte-for-byte identical to the previous inline strings via a throwaway Node read.
- [X] T013 [P] [US1] [US3] Drop the two-question business form and `os/identity.md` creation; show MCP-connection guidance instead of a bare "already exists" message (FR-004, FR-006, FR-008 through FR-010, research.md §10), per user request after replacing `frontend/lib/os/templates/init.md` with a much richer skill that runs its own interview and writes `os/identity.md` itself. Changes: `initializeCompanyOs()` (T001, `lib/os/init.ts`) takes no arguments and no longer creates `os/identity.md`; removed `frontend/app/init/InitForm.tsx`; `frontend/app/init/submit/route.ts` (T004) no longer reads `request.formData()`; `frontend/app/init/page.tsx`'s `"empty"` branch (T005) is now a single no-field `<form>`/button; created `frontend/app/init/McpConnectManual.tsx` (MCP server URL derived from `headers()`, OAuth connection notes, `/settings/connected-apps` and `/editor` links, `justCreated` prop for the `?created=1` case) replacing the old bare message in the `"already_initialized"` branch (T008). `npx tsc --noEmit` clean.
