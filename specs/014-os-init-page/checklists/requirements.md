...
- 2026-07-25 (post-implementation revision): removed the two-question business form and `/os/identity.md` creation entirely (FR-005, FR-007 marked removed) — the connected AI assistant's own init skill now runs that interview. The "already initialized" state (FR-003, US3) now shows MCP-connection/OAuth guidance for Claude/ChatGPT instead of a bare "already exists" message, and User Story 1's confirmation step shows the same guidance immediately after creating the skeleton. Re-validated — still 16/16 passing; no new implementation-detail leakage (mentioning Claude/ChatGPT/OAuth by name is intrinsic to the feature, already established by specs 008's own spec.md).
