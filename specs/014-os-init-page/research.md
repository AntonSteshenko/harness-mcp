...
**Decision**: `initializeCompanyOs()` (new, `lib/os/init.ts`) re-runs the §3 existence check as its first step and returns an "already initialized, nothing done" result instead of writing anything if either `os/` or `data/` already exists by the time it runs — not just relying on the page's own pre-render check. Then it creates, in order: `os/` (`createDirectory`), `data/` (`createDirectory`), `os/identity.md` (`createFile`), `AGENTS.md` (`createFile`), `os/skills/init.md` (`createFile`).

**Rationale**: The page's state check (§3) and the form submission are two separate requests, so a double-submit or two near-simultaneous first-time visits create a race between "check" and "act." Re-checking inside `initializeCompanyOs()` narrows that window to the check-then-write sequence itself rather than the much larger check-then-user-clicks-submit window, and guarantees a second concurrent call is a no-op rather than a silent overwrite — meeting SC-004 ("0% of submissions against an already-initialized... bucket result in existing content being overwritten") for the realistic single-owner traffic this app serves. A distributed lock is not warranted at this app's scale (spec.md Assumptions/Scale).
...
**Decision**: The `AGENTS.md` and `os/skills/init.md` template text are plain string constants inside `lib/os/init.ts`, substituted with nothing but the two form answers is not required (`AGENTS.md`/`init.md` are fixed; only `os/identity.md` is templated with the two answers).

**Rationale**: Both files are explicitly "fixed, product-provided" per spec.md (FR-008, FR-009, Assumptions) — no per-business customization, no reason to read them from an external file or a database. Keeping them as source-code constants means they ship with the app binary/deployment like any other code, with no extra moving part.

## §6. Reusing the post-submit confirmation as the "already exists" view

**Decision**: On successful submission, `/init/submit`'s `POST` handler redirects (`303`) back to `/init` rather than rendering a bespoke confirmation page. Immediately after redirect, `/init`'s own state check (§3) now finds both `os/` and `data/` present and renders the same "already initialized" view FR-003/US3 already defines — which already satisfies FR-010's "confirmation and a link to `/editor`."

**Rationale**: Avoids a fourth, one-off page state that would duplicate almost all of the "already exists" view's content. The two are meaningfully the same message ("a Company OS exists, here's the editor link") from the visitor's point of view.

**Alternatives considered**: A dedicated success page/banner distinct from the "already exists" view → rejected as unnecessary duplication; a `?created=1` query param can optionally tweak copy (e.g. "Your Company OS is ready" vs. "A Company OS already exists") without introducing a new state or route.
