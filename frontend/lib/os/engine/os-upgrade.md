...
self-triggers — unlike the `get_os_init` tool's business-data check, this only
runs when asked.
...
1. `read_file "AGENTS.md"`. Read its `os-engine-version` front matter field
   (absent → version `0`).
2. Call the `get_os_engine` tool. Read its current `os-engine-version` and its
   `## Changelog` section.
3. Compare:
   ...
   - **Recorded version is behind current** → this is exactly the situation
     `get_os_engine`'s own **Repair** procedure already handles (collect
     ...
     independent ones.
   - **No `os-engine-version` recorded at all (v0)** → also defer to
     `get_os_engine`, which handles the v0 case (routing-table extraction
