...
Self-contained instructions for the only thing this resource is responsible for:
`AGENTS.md`, the Company OS's single router file. It never touches `data/`,
`os/identity.md`, `os/policies/*`, domain skills, or `os/routing.md` — that's the
`init` resource's job (spec 016). This resource is reached automatically whenever
`AGENTS.md` needs to be created or repaired; an owner never asks for it by name.
...
2. Read `AGENTS.md`'s front matter. Does it already carry `os-engine-version`?
   - **Yes, and it matches this resource's current `os-engine-version`** → nothing
   ...
   - **Yes, but it's older than this resource's current `os-engine-version`** →
...
3. `init` never touches an already-populated `data/`, `os/identity.md`,
   `os/policies/*`, or any domain skill file — those belong to the `init`
...
2. Front matter: `os-engine-version: 1` (this resource's current version, never
...
3. Body, in `os/language` (read that file; if it doesn't exist, use English):
   ...
   - Point to `os/routing.md` for "which skill handles what" — **do not** embed
     ...
     build; that's fine, it's the `init` resource's job to create it, and
   ...
   - Keep one line pointing whatever assistant reads this next at the `init`
     resource, for the business setup / repair / extend / start-over flows.
4. Do not write anything under `data/`, `os/identity.md`, `os/policies/*`, or
   any domain skill file here — defer entirely to `init`.
...
1. If `AGENTS.md`'s recorded `os-engine-version` already matches this
   resource's current version: rebuild directly from **Build** above. No
...
4. This confirm-then-rebuild procedure is the single gate both a repair and an
   explicit upgrade check go through — the `os-upgrade` resource, when an owner
...
4. Never skip the extraction to save a step — losing routing entries here is
   the one mistake this resource must never make.
...
- Initial versioned engine. `AGENTS.md` gains `os-engine-version`; the routing
  ...
  interview and everything it produces moves to the `init` resource.
